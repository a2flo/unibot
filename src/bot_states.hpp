...
public:
	...
	struct user_info {
		...
		user_info() : nick(""), real_name(""), host(""), host_user(""), registered(""), ctcp_support(""), client("") {}
		user_info(const string& nick_, const string& real_name_, const string& host_, const string& host_user_, const string& registered_,
				  const string& ctcp_support_, const string& client_) :
		nick(nick_), real_name(real_name_), host(host_), host_user(host_user_), registered(registered_), ctcp_support(ctcp_support_), client(client_) {}
		~user_info() {}
	};
	...
	void update_user_info(string name, string real_name = "", string host = "", string host_user = "", string registered = "", string ctcp_support = "",
						  string client = ""); // "" == don't update
